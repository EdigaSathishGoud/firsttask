...
def load_conversations():
    try:
        with open("conversations_pdf.json", "r") as file:
...
def save_conversations(conversations):
    with open("conversations_pdf.json", "w") as file:
        json.dump(conversations, file)

# Load saved conversations
conversations = load_conversations()

def llm_response(pdf_file,query):
    client = AzureOpenAI(
    ...
    response = client.chat.completions.create(
        ...
        messages=[        
            {"role": "user", "content": f"""You are an expert in answering user queries only related to {pdf_file} 
             \n A query to answer: {query} 
             \n If you dont know the answer or query is not related to pdf ,reply as I Don't Know """}
        ],
        temperature=0,
        max_tokens=1024
    )
...
if prompt := st.chat_input("Enter text to answer"):
    ...
    if prompt in conversations:
        ...
        response = llm_response(pdf_file,prompt)
