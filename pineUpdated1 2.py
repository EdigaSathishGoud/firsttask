...
def run_query(query):
    """
    Run an SQL query on specific dataframes.
    
    :param query: SQL query to execute.
    :param dataframes: Dictionary of dataframes to be included in the query.
    :return: Result of the SQL query.
    """
    dfs = {'orders': orders, 'delivery': delivery, 'billing': billing}
    return psql.sqldf(query, dfs)
...
text = f"""There are sql tables named orders with it's properties orders({orders_schema},
    \n (DOC_NUMBER and S_ORD_ITEM are primary keys), \n delivery with it's properties delivery({delivery_schema}, \n 
    DELIV_NUMB and DELIV_ITEM are primary keys , \n SalesOrderDocument and SalesOrderItem are foreign keys referring DOC_NUMBER and S_ORD_ITEM from orders), \n 
    billing with it's properties billing({billing_schema}, \n BILL_NUM and BILL_ITEM are primary keys, \n DELIV_NUMB and DELIV_ITEM are foreign keys referring DELIV_NUMB and DELIV_ITEM from delivery) 
...
def get_conversational_chain(vector_store):
    ...
    Generate SQL query related to SQLite database format for the given question.
    In the mapping dataframe provided in the context you will find the simplified names for each original name in every row with its description. Simplified names can be used in the
    question but during the sql query generation you have to use original names for that respective simplified names. 
    Don't use columns which in not in the table for given three tables. Don't use any names and methods with is not correct in query language.
    sql code should not have ``` in beginning or at the end of sql output and there shouldn't be any explanation for the generated query. Give only the correct sql query and no text or explanation in the output.
...
def main3():
    ...
    if st.session_state.show_faqs:
        st.session_state.selected_question = st.sidebar.selectbox("Select a FAQ", faq, key="faq_selectbox", index=None, placeholder="Choose a question")
        # if st.sidebar.button("Ask FAQ"):
        st.session_state.user_question = st.session_state.selected_question

    # Text input for user questions
    st.session_state.user_question = st.text_input("Chatbot", value=st.session_state.get("user_question", ""), key="user_input")

    if st.button("Submit Question"):
        st.session_state.process_question = st.session_state.user_question

    if "process_question" in st.session_state:
        user_question = st.session_state.process_question
        response = {}
        # Load the model
        ...
        if user_question:
            ...
            if score > 0.8:
                ...
                st.session_state.sqlquery = response['content']
            else:
                response = st.session_state.conversation({'question': user_question})  # Start a conversation with a user's question
                index.upsert(vectors=[{'id': user_question, 'values': model1.encode(user_question).tolist(), 'metadata': {'Question': user_question, 'Answer': response['answer'], 'count': 1}}], namespace=username)
                st.session_state.chatHistory.append({'role': 'user', 'content': user_question})
                st.session_state.chatHistory.append({'role': 'bot', 'content': response['answer']})
                st.session_state.sqlquery = response['answer']

            del st.session_state.process_question  # Reset process_question after it's processed
    ...
    # st.dataframe(df1)
    # sqlquery = "SELECT SALESORG, COUNT(DISTINCT DOC_NUMBER) AS count_sales_order_header FROM orders"
    if len(st.session_state.sqlquery) >2:
        df = run_query(st.session_state.sqlquery)
        st.dataframe(df)

    # print('\n\n---------------------------************----------------------' + {st.session_state.user_question or ''} + '\n\n---------------------------************----------------------')
