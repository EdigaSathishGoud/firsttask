...
def main():
    ...
    if choice == "Bulk load":
        ...
        if uploaded_file:
            ...
            os.makedirs("ebcdic_output",exist_ok=True)
            ...
            if st.button("Generate Data", key="generate_data_button"):
                ...
                for i in range(df.shape[0]):
                    ...
                    if prompt:
                        ...
                        data_df.to_csv(Path("output") / f"generated_{path}")
                        input_csv = Path("output") / f'generated_{path}'

                        output_ebcdic =Path("ebcdic_output")/f'generated_ebcdic_{path}'
            ...
            if st.button("Process Data"):
                ...
                if final_prompt:
                    ...
                    if uploaded_file is not None:
                        ...
                        data_df.to_csv(f"generated_{uploaded_file.name}")
                        input_csv = f'generated_{uploaded_file.name}'
                        output_ebcdic = f'generated_ebcdic_{uploaded_file.name}'
                        convert_csv_to_ebcdic(input_csv, output_ebcdic)
                        
                        st.write(data_df)  
                        col5,col6 = st.columns(2)
                        with col5:
                            with open(output_ebcdic, "rb") as file:
                                st.download_button(label="Download EBCDIC", data=file, file_name=output_ebcdic, mime="text/csv") 
                                
                        with col6:
                            with open(input_csv, "rb") as file:
                                st.download_button(label="Download CSV", data=file, file_name=input_csv, mime="text/csv") 
